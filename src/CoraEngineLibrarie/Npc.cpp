...
float Npc::get_direction() const
{
	return direction;
}

float Npc::get_distance() const
{
	return distance;
}
...
{
	float sprite_height = sprite_manager->get_sprite_data("AMO").texture_box.height;
	float sprite_width = sprite_manager->get_sprite_data("AMO").texture_box.width;
...
void Npc::draw(const short i_pitch, sf::RenderWindow& i_window)
{
	...
	{
		sprite_manager->draw_sprite(current_frame, "AMO", sf::Vector2<short>(get_x(), i_pitch + get_y()), i_window, 0, 0, get_width() / sprite_width, get_height() / sprite_height, sf::Color(shade, shade, shade));
	}
}
...
void Npc::update(const sf::RenderWindow& i_window, const sf::Vector2f& i_player_direction, const sf::Vector2f& i_player_position, const gbl::MAP::Map<>& i_map)
{
	if (1 == i_window.hasFocus())
	{
		...
		float frame_angle = 360.f / sprite_manager->get_sprite_data("AMO").total_frames;
