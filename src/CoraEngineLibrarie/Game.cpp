...
Game::Game() :
	...
	hand_animation("WEAPON_2", sprite_manager, 0, gbl::SPRITES::FIRE_ANIMATION_SPEED)
{
	window.setMouseCursorVisible(0);
	window.setView(sf::View(sf::FloatRect(0, 0, gbl::SCREEN::WIDTH, gbl::SCREEN::HEIGHT)));
	map = convert_map_sketch(*managerMap.GetLevel(0), props, player, npc, sprite_manager);
	for (Npc& npcit : npc)
		npcit.fill_map(map);
...
void Game::draw()
{
		...
		for (unsigned short a = floor_start_y; a < gbl::SCREEN::HEIGHT; a++)
		{
			...
			if (0 < shade)
			{
				...
				{
					sf::Color floor_image_pixel = floor_image.getPixel(floor(floor_sprite_data.texture_box.width * (floor_sprite_data.index + floor_x - floor(floor_x))), floor(floor_sprite_data.texture_box.height * (floor_y - floor(floor_y))));
		...
		for (Stripe& stripe : stripes)
		{
			while (decoration_index < props.size() && stripe.get_distance() < props[decoration_index].get_distance())
			{
				...
				{
					for (Npc& steven : npc) {
						if (steven.get_distance() > props[decoration_index].get_distance())
						{
							npc_is_drawn = 1;
				...
				props[decoration_index].draw(pitch, window);
		...
		for (unsigned short a = decoration_index; a < props.size(); a++)
		{
			if (0 == npc_is_drawn)
			{
				for (Npc& steven : npc) {
					if (steven.get_distance() > props[a].get_distance())
					{
						npc_is_drawn = 1;
		...
		hand_animation.draw(sf::Vector2<short>((gbl::SCREEN::WIDTH + hand_offset_x - hand_texture_width)/2, (gbl::SCREEN::HEIGHT + hand_offset_y - hand_texture_height)* 2.75f), window, 0, 0, gbl::SPRITES::HAND_SCALE, gbl::SPRITES::HAND_SCALE);
...
void Game::draw_map()
{
	...
	for (Npc& steven : npc) {
		...
		sprite_manager.draw_sprite(floor(steven_shifted_direction / frame_angle), "MAP_NPC", sf::Vector2<short>(round(gbl::MAP::CELL_SIZE * steven.get_position().x), round(gbl::MAP::CELL_SIZE * steven.get_position().y)), window);
	}
...
void Game::update()
{
	bool isScreamer = false;
		...
		for (Prop& prop : props)
		{
			prop.update(player.get_direction(), player.get_position());
		}
